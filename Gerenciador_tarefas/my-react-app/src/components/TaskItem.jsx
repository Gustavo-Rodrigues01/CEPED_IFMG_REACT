   ...
   return(
    <div className="text-3xl w-500px bg-blue-200 flex flex-col p-4 space-y-2 rounded-2xl">
        ...
        <div key={tasks.id} className="flex space-x-2">
            ...
            <button onClick={() => taskComplet(tasks.id)} className="bg-white text-green-600 rounded-md w-[60px] h-[50px]">V</button>
            <button onClick={() => taskDelet(tasks.id)} className="bg-white text-red-600 rounded-md w-[60px] h-[50px]">X</button>
        </div>
           ))}</div>
    </div>
